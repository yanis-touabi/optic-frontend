  ...
  <table className="w-full border-collapse text-[12px] mb-4">
    <thead>
      <tr className="bg-gray-100">
        ...
        <th className="border border-black px-2 py-1.5 w-16 text-center">
          Qté
        </th>
        <th className="border border-black px-2 py-1.5 w-28 text-right">
          P.U.
        </th>
        <th className="border border-black px-2 py-1.5 w-32 text-right">
          Prix
        </th>
      </tr>
    </thead>
      ...
      {cmd.lignes.map((l) => (
        <tr key={l.id}>
          ...
          <td className="border border-black px-2 py-1.5 text-right">
            {fmtPrice(l.prixUnitaire)}
          </td>
          <td className="border border-black px-2 py-1.5 text-right">
            {fmtPrice(l.quantite * l.prixUnitaire)}
          </td>
        </tr>
      ))}
      {Array.from({ length: Math.max(0, padRows - cmd.lignes.length) }).map(
        (_, i) => (
          <tr key={`e${i}`}>
  ...
  return (
    <div className="font-sans text-[13px] text-black">
      ...
      <div className="grid grid-cols-2 gap-6 mt-12 text-[11px]">
        <div>
          <div className="border-t border-black pt-1">Signature client</div>
        </div>
        <div className="text-right">
          <div className="border-t border-black pt-1">Cachet & signature</div>
        </div>
      </div>
...
export function BonModerne({ cmd, client, ord, store }: Props) {
  return (
    <div className="font-sans text-[13px] text-black">
      <div
        className="flex justify-between items-center mb-6 -mx-10 -mt-10 px-10 py-6 print:mx-0 print:mt-0 print:px-6 print:py-4"
        style={{
