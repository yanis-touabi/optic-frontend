...
import { Eye, Glasses, Users, FileText, ShoppingBag, ClipboardList, LayoutDashboard, LogOut, UserCircle, ShieldCheck } from "lucide-react";
...
type NavItem = { to: string; label: string; icon: typeof LayoutDashboard; end?: boolean; role?: "ADMIN" | "OPTICIEN" };

const nav: NavItem[] = [
  { to: "/", label: "Tableau de bord", icon: LayoutDashboard, end: true },
  { to: "/commandes", label: "Bons de commande", icon: ShoppingBag, role: "OPTICIEN" },
  { to: "/commandes/nouveau", label: "Nouveau bon", icon: ClipboardList, role: "OPTICIEN" },
  ...
  { to: "/admin/utilisateurs", label: "Utilisateurs", icon: ShieldCheck, role: "ADMIN" },
];
...
export default function AppLayout() {
  ...
  const visibleNav = nav.filter((n) => !n.role || (n.role === "ADMIN" ? isAdmin : isManager));
      ...
      <aside className="no-print w-64 shrink-0 bg-sidebar text-sidebar-foreground border-r border-sidebar-border flex flex-col">
        <div className="px-6 py-6 border-b border-sidebar-border flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-xs text-sidebar-foreground/60">Gestion opticien</div>
          </div>
        </div>
        ...
        <div className="px-3 py-3 border-t border-sidebar-border space-y-2">
          {user && (
            <div className="px-3 py-1 space-y-1">
              <div className="text-sm font-medium text-sidebar-foreground truncate" title={user.profile?.displayName || "Utilisateur"}>
          ...
          <div className="px-3 text-[10px] text-sidebar-foreground/40">v1.0</div>
        </div>
      </aside>
