...
export default function Dashboard() {
  ...
  // ── Stock alerts ──
  const outOfStock = useMemo(() => allProduits.filter((p) => p.stock <= 0), [allProduits]);
  const lowStock = useMemo(() => allProduits.filter((p) => p.stock > 0 && p.stock <= 2), [allProduits]);
  const hasStockAlerts = outOfStock.length > 0 || lowStock.length > 0;
