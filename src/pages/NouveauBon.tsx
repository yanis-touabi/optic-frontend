...
import { useClients, useOrdonnances, useProduits, useCreateCommande } from "@/lib/data";
...
export default function NouveauBon() {
  ...
  const ordonnancesClient = ordonnances.filter((o) => o.clientId === effectiveClientId);
  ...
  const stockIssues = useMemo(() => checkStock(lignes, produits), [lignes, produits]);
  ...
  return (
    <>
      <PageHeader
        ...
        actions={
          <>
            <Button variant="outline" onClick={() => save(false)} disabled={saving || stockIssues.length > 0}>
              {saving && <Loader2 className="h-4 w-4 animate-spin" />}Enregistrer
            </Button>
            <Button onClick={() => save(true)} disabled={saving || stockIssues.length > 0}>
              <Printer className="h-4 w-4" />Enregistrer & imprimer
            </Button>
          </>
        }
      />
      <div className="p-8 grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
          ...
          <Card className="shadow-[var(--shadow-card)]">
            ...
            <CardContent>
              <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Observations, instructions..." />
            </CardContent>
          </Card>
        </div>
