...
export default function EditerBon() {
  ...
  }, [produits, selectedProduits, cmd, stockLocked]);

  const stockIssues = useMemo(
    () => checkStock(lignes, effectiveProduits),
    [lignes, effectiveProduits],
  );
  ...
  const removeLigne = (lid: string) =>
    setLignes((l) => l.filter((x) => x.id !== lid));
  ...
  return (
    <>
      <PageHeader
        ...
        description={
          stockLocked
            ? 'Le stock est déjà appliqué — modifications sans contrôle de stock.'
            : 'Modifier les informations du bon'
        }
        actions={
          <>
            <Button variant="outline" onClick={() => nav('/commandes')}>
              Annuler
            </Button>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="overflow-x-auto">
                <Table className="table-fixed w-full min-w-[600px]">
                  <TableHeader>
                    <TableRow>
                      <TableHead className="w-[45%]">Désignation</TableHead>
