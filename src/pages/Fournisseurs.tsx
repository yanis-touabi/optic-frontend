...
function SupplierProductsModal({
  ...
  const [type, setType] = useState<'ALL' | 'MONTURE' | 'VERRE' | 'ACCESSOIRE'>('ALL');
  const [page, setPage] = useState(0);
  const [pageSize, setPageSize] = useState(10);
  ...
  const { data: productsData, isLoading: isProductsLoading } = useSupplierProducts(
    supplier?.id,
    {
      page,
      limit: pageSize,
                            ...
                            .map((p) => (
                              <CommandItem
                                ...
                                onSelect={() => {
                                  if (!supplier) return;
                                  linkMutation
                                    .mutateAsync({ fournisseurId: supplier.id, produitId: p.id })
                                    .then(() => {
              ...
              <div className="mt-4 space-y-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(['ALL', 'MONTURE', 'VERRE', 'ACCESSOIRE'] as const).map((option) => (
                      <Button
                        key={option}
                        type="button"
