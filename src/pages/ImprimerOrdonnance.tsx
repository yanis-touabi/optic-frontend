...
import { useClients, useOrdonnances, useStore } from '@/lib/data';
import {
  OrdonnanceClassique,
  OrdonnanceCompact,
  OrdonnanceDetaille,
} from '@/components/print/OrdonnanceTemplates';
import { TemplateSelect } from '@/components/TemplateSelect';
import {
  ordonnanceTemplates,
  getOrdonnanceTemplate,
  setOrdonnanceTemplate,
...
export default function ImprimerOrdonnance() {
  ...
  useEffect(() => {
    if (ord) document.title = `Ordonnance`;
  }, [ord]);
  ...
  if (!ord) {
    return (
      <div className="p-10 text-center">
        ...
        <Button onClick={() => nav('/ordonnances')} className="mt-4">
          Retour
        </Button>
      </div>
    );
  ...
  const onDownload = async () => {
    ...
    try {
      await downloadElementAsPdf(
        printRef.current,
        `ordonnance-${ord.id.slice(0, 8)}.pdf`,
      );
    } catch (e: unknown) {
    ...
    <div className="min-h-screen bg-muted/40 print:bg-white">
      ...
      <div className="py-6 print:py-0 flex justify-center overflow-auto">
        ...
        >
          ...
          >
            <Render ord={ord} client={client} store={store} />
          </div>
        </div>
      </div>
    </div>
