...
export default function ImprimerBon() {
  ...
  useEffect(() => {
    if (cmd) document.title = `Reçu N° ${cmd.numero}`;
  }, [cmd]);
  ...
  if (!cmd) {
    return (
      <div className="p-10 text-center">
        ...
        <Button onClick={() => nav('/commandes')} className="mt-4">
          Retour
        </Button>
      </div>
    );
  ...
  const ord = cmd.ordonnanceId
    ? ordonnances.find((o) => o.id === cmd.ordonnanceId)
    : undefined;

  const onTpl = (v: string) => {
    setTpl(v as BonTemplate);
    setBonTemplate(v as BonTemplate);
  };
  const Render =
    tpl === 'compact'
      ? BonCompact
