...
import { BonClassique, BonCompact, BonModerne } from "@/components/print/BonTemplates";
import { TemplateSelect } from "@/components/TemplateSelect";
import { bonTemplates, getBonTemplate, setBonTemplate, type BonTemplate } from "@/lib/templates";
import { downloadElementAsPdf } from "@/lib/pdf";
import { toast } from "sonner";

export default function ImprimerBon() {
  const { id } = useParams();
  ...
  useEffect(() => { if (cmd) document.title = `Reçu N° ${cmd.numero}`; }, [cmd]);
  ...
  if (!cmd) {
    return (
      <div className="p-10 text-center">
        <p className="text-muted-foreground">Bon introuvable.</p>
        <Button onClick={() => nav("/commandes")} className="mt-4">Retour</Button>
      </div>
    );
  }

  const client = clients.find((c) => c.id === cmd.clientId);
  const ord = cmd.ordonnanceId ? ordonnances.find((o) => o.id === cmd.ordonnanceId) : undefined;

  const onTpl = (v: string) => { setTpl(v as BonTemplate); setBonTemplate(v as BonTemplate); };
  const Render = tpl === "compact" ? BonCompact : tpl === "moderne" ? BonModerne : BonClassique;

  const onDownload = async () => {
    if (!printRef.current) return;
    setDownloading(true);
    try {
      await downloadElementAsPdf(printRef.current, `bon-${cmd.numero}.pdf`);
    } catch (e: unknown) {
    ...
    <div className="min-h-screen bg-muted/40 print:bg-white">
      <div className="no-print bg-card border-b px-6 py-3 flex flex-wrap justify-between items-center gap-3 sticky top-0 z-10">
        ...
        <div className="flex items-center gap-2">
          <TemplateSelect value={tpl} onChange={onTpl} options={bonTemplates} />
